...
{
    /// <summary>
    /// Adds the "Create Scene Content" button to <see cref="URPBasicSceneSetup"/>.
    ///
    ...
    /// </summary>
    [CustomEditor(typeof(URPBasicSceneSetup))]
    internal sealed class URPBasicSceneSetupEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(Application.isPlaying))
            {
                if (GUILayout.Button("Create Scene Content", GUILayout.Height(24)))
                    CreateContent((URPBasicSceneSetup)target);
            }
        }

        private static void CreateContent(URPBasicSceneSetup setup)
        {
            var scene = setup.gameObject.scene;
            if (!scene.IsValid() || string.IsNullOrEmpty(scene.path))
            {
                Debug.LogWarning("[URPBasicSceneSetup] Save the scene first — the materials are " +
            ...
            if (folder == null)
            {
                Debug.LogWarning("[URPBasicSceneSetup] Could not work out where to put the materials " +
            ...
            Debug.Log($"[URPBasicSceneSetup] Authored {count} objects into '{scene.name}', " +
        ...
        {
            string path = $"{folder}/URPBasicScene_{ColorUtility.ToHtmlStringRGB(color)}.mat";
            var existing = AssetDatabase.LoadAssetAtPath<Material>(path);
            if (existing != null) return Tint(existing, color);

            // Same resolution and warning as the runtime path, so authoring in a
            // project without URP set up fails the same visible way rather than
            // silently producing Standard-shaded assets.
