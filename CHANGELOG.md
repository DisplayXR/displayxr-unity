...
- **The Window-Space UI overlay was squeezed under the shell whenever the workspace tile's aspect differed from the player window's (#323).** `DisplayXRWindowSpaceUI` derived the live panel aspect from `Screen.width`/`Screen.height`, but under the shell Unity's main window is launched minimized and never follows the tile — so `Screen.*` stayed frozen at the launch size while the **eye** swapchain tracked the tile (the provider re-polls the tile canvas every reconcile). The overlay RT was therefore never re-created, and the runtime's stretch into the fractional panel rect stopped being the identity: the tuning panel's text came out horizontally squeezed in a narrow tile. The provider's tile canvas is now exposed to managed code as `displayxr_get_render_canvas_size` and preferred over `Screen.*` in shell mode; the existing settle logic handles tile drag-resizes exactly as it handles window resizes. One log line records which size source is in use (`wsui: panel size source = …`). Standalone runs and every non-shell path are unchanged — the export returns 0 there and `Screen.*` is used as before.
